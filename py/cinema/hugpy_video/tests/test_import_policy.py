"""Structural import policy for hugpy_video (generated from partition.toml).
...
ALLOWED = ['hugpy_platform', 'hugpy_control', 'hugpy_engine', 'hugpy_media']
OPTIONAL = []
...
def test_package_imports_without_monolith():
    blocked = ["abstract_hugpy_dev", *OPTIONAL]
    code = "; ".join(
        ["import sys"]
        + [f"sys.modules[{name!r}] = None" for name in blocked]
        + [f"import {PACKAGE}", f"print({PACKAGE}.__name__)"]
    )
