"""Structural import policy for hugpy_engine (generated from partition.toml).
...
PACKAGE = "hugpy_engine"
PACKAGE_ROOT = Path(__file__).resolve().parents[1] / "src" / PACKAGE
ALLOWED = ['hugpy_platform', 'hugpy_control', 'hugpy_storage']
OPTIONAL = []
...
def test_package_imports_without_monolith():
    code = "; ".join(
        ...
        + [f"sys.modules[{name!r}] = None" for name in OPTIONAL]
