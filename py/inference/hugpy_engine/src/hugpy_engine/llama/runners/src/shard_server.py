...
def ensure_shard_server(
    ...
    with _LOCK:
        ...
        if ts_key:
            # ORDER FIX: llama.cpp maps --tensor-split to devices as
            # [rpc-backends..., local lead] (RPC devices first, lead last), but
            # the allocator emits tensor_split as [lead, *rpc]. Without rotating
            # the leads share to the end, a small backend GPU gets the leads
            # large slice and OOMs (proven: split 0.7,0.3 -> backend got 0.7).
            _ts = list(ts_key)
            _ordered = _ts[1:] + _ts[:1] if len(_ts) > 1 else _ts
            cmd += ["--tensor-split", ",".join(str(x) for x in _ordered)]
