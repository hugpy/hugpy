...
from abstract_essentials import get_env_value, get_logFile, make_list
from hugpy_engine.config.main import get_model_config, get_model_path, resolve_model_source
from hugpy_engine.config.models.models_config import get_model_registry
from hugpy_platform.constants import LLAMA_HOST
...
from hugpy_platform.binaries import with_exe as _with_exe
from hugpy_platform.app_dirs import engine_dir as _engine_dir
from hugpy_engine.native import resolve as _engine_resolve
...
from hugpy_platform.platform_facade import IS_LINUX as _IS_LINUX
...
def _ctx_for(cfg, model_key, extra=None):
    ...
    if _CTX_RESOLVER is not None:
        try:
            ...
            if resolved:
                logger.debug("%s: serving -c %s (ctx allocation — ctx_pct)",
...
class ServeSpec:
    ...
    def slot_n_gpu_layers(self):
        ...
        try:
            from hugpy_engine.serve.slot_agent import _NglDefaulted
...
def _ngl_for_alloc_mode(alloc_mode, model_file, extra) -> "int | None":
    ...
    from hugpy_engine.alloc_modes import resolve_alloc_mode
...
class SupervisedDriver:
    ...
    def _cmd(self, action, model_key):
        return RunCmd((sys.executable, "-m", "hugpy_engine.serve.supervisor",
                       action, model_key))
...
def serving_overview(registry=None):
    ...
    2026-09-18: each row is annotated ``present`` = whether central has the
    model's weights on disk (the ``model_physical`` store — the post-migration
    source of truth). The registry lists every CONFIGURED model, including ones
    nothing has downloaded yet, so the console can grey-out / defer models that
    aren't actually servable instead of showing them as ready.
    """
    registry = registry if registry is not None else get_model_registry()
    ...
    for key, spec in specs.items():
        ...
        row = spec_row(spec, driver)
        row["present"] = _central_has_weights(key, registry.get(key))
        rows.append(row)
    return rows


def _central_has_weights(model_key, cfg) -> bool:
    """True if central's model_physical store has this model's weights on disk.
    Never raises — presence is advisory, a store hiccup degrades to False."""
    if cfg is None:
        return False
    try:
        from hugpy_storage.model_physical import lookup_physical
        rec, _reason = lookup_physical(model_key, cfg)
        return rec is not None
    except Exception:  # noqa: BLE001 — never break the overview on a presence lookup
        return False
